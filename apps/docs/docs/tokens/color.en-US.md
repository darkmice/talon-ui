...
All color values are read directly from `packages/tokens/src/tokens.css`. The Primitive layer is theme-stable; the Semantic layer switches between light and dark.

---

## Neutral Ramp

`--tp-gray-*` is the raw material for all backgrounds, borders, and text colors.

| Token | Swatch | Light value |
| --- | --- | --- |
...
Talon brand Indigo Blue. Theme-stable (same value in light and dark).
...
| `--tp-primary-500` | <span style="display:inline-block;width:24px;height:24px;border-radius:4px;background:#4f60ff;border:1px solid rgba(0,0,0,0.06);vertical-align:middle;"></span> | `#4f60ff` ← Brand |
...
Tailwind: `bg-primary-500` / `text-primary-600` / `border-primary-200` etc.

---

## Functional Ramps

Each functional color provides 100 (light background) / 300 (mid-tone) / 500 (foreground) for assembling status tokens.
...
## Accent (Avatar / Persona)

Six accent groups, each with three values: main / soft background / ink. For avatar backgrounds and skill chips only — not for general UI.
...
Tailwind: `bg-accent-violet` / `text-accent-violet-ink` / `bg-accent-orange-soft` etc.

---

## Danger (Brand Red)

Talon brand red, matching `--tp-red-500`. Theme-stable — does not change in dark mode.
...
Each status has a `fg` (foreground text/icon) and `bg` (background) token — always use them together.
...
Tailwind shorthand (sets fg + bg together): `.tp-status-done` / `.tp-status-progress` / `.tp-status-pending` / `.tp-status-blocked` / `.tp-status-idle` / `.tp-status-info`.
